calendarFlow <- function(dates, values, date.form = "%Y-%m-%d", span=NA, palette="red", main=NA) {
   ...
   # Month lines
    ...
    # Title
    if (!is.na(main)) {
        fulltitle = paste(main, ": ", startDate, "to", endDate)
    	text(1.5, 8, fulltitle, pos=4, cex=0.8)
    } else {
    ...
    for (k in 1:7) {
        text(0, k-1/2, daylabs[k], cex=0.4)
    }

    # Month labels
    monthlabs <- c('Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec')

    currMonth = month(startDate) - 1
    currYear = year(startDate)
    for (k in 0:weeknum) {
        currDate = dates[1 + 7*k]
        if ((month(currDate) > currMonth) | (year(currDate) > currYear)) {
            currMonth = month(currDate)
            currYear = year(currDate)
            text(k+2, -1, monthlabs[currMonth], cex=0.6)
        }
    }
}
